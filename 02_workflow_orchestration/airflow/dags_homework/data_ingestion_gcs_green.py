...
GREEN_TRIP_SOURCE_DATASET_FILE = TRIPDATA_URL_PREFIX + "green_tripdata_{{ execution_date.strftime('%Y-%m') }}.parquet"
GREEN_TRIP_TARGET_DATASET_FILE = "green_tripdata_{{ execution_date.strftime('%Y-%m') }}.parquet"
...
with DAG(
    dag_id='green_taxi_data_pipeline',
    ...
    download_dataset_task = BashOperator(
        ...
        bash_command=f"curl -sSL {GREEN_TRIP_SOURCE_DATASET_FILE} > {PATH_TO_LOCAL_HOME}/{GREEN_TRIP_TARGET_DATASET_FILE}"
    )
    ...
    convert_to_parquet_task = PythonOperator(
        ...
        op_kwargs={
            "src_file": f"{PATH_TO_LOCAL_HOME}/{GREEN_TRIP_TARGET_DATASET_FILE}",
        }
    )
    ...
    local_to_gcs_task = PythonOperator(
        ...
        op_kwargs={
            "bucket": BUCKET,
            "object_name": f"hw/green_taxi/{{{{ execution_date.strftime('%Y') }}}}/{GREEN_TRIP_TARGET_DATASET_FILE}",
            "local_file": f"{PATH_TO_LOCAL_HOME}/{GREEN_TRIP_TARGET_DATASET_FILE}",
        },
    )
    ...
    #             "sourceUris": [f"gs://{BUCKET}/hw/green_taxi/{{{{ execution_date.strftime('%Y') }}}}/{GREEN_TRIP_TARGET_DATASET_FILE}"],
    #         },
    #     },
    # )
