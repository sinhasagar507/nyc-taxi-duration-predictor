...
FHV_TRIP_SOURCE_DATASET_FILE = TRIPDATA_URL_PREFIX + "fhv_tripdata_{{ execution_date.strftime('%Y-%m') }}.parquet"
FHV_TRIP_TARGET_DATASET_FILE = "fhv_tripdata_{{ execution_date.strftime('%Y-%m') }}.parquet"
FHV_TRIP_BIGQUERY_DATASET = os.environ.get("FHV_TRIP_BIGQUERY_DATASET", "fhv_trips_data")
...
with DAG(
    ...
    download_dataset_task = BashOperator(
        ...
        bash_command=f"curl -sSL {FHV_TRIP_SOURCE_DATASET_FILE} > {PATH_TO_LOCAL_HOME}/{FHV_TRIP_TARGET_DATASET_FILE}"
    )
    ...
    convert_to_parquet_task = PythonOperator(
        ...
        op_kwargs={
            "src_file": f"{PATH_TO_LOCAL_HOME}/{FHV_TRIP_TARGET_DATASET_FILE}",
        },
    )
    ...
    local_to_gcs_task = PythonOperator(
        ...
        op_kwargs={
            "bucket": BUCKET,
            "object_name": f"hw/fhv_taxi/{{{{ execution_date.strftime('%Y') }}}}/{FHV_TRIP_TARGET_DATASET_FILE}",
            "local_file": f"{PATH_TO_LOCAL_HOME}/{FHV_TRIP_TARGET_DATASET_FILE}",
        },
    )
